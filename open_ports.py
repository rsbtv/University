...
TARGET_USER = "tsmts"
TARGET_PORTS = ["ttyAP0", "ttyAP1", "ttyAP2", "ttyAP3"]
...
def create_udev_rule(devices):
    ...
    ports_str = ", ".join(TARGET_PORTS)
    rules = ['# Постоянный доступ к COM-портам /dev/%s' % ports_str]

    # Правила конкретно для наших портов
    for port in TARGET_PORTS:
        rules.append(
            'SUBSYSTEM=="tty", KERNEL=="%s", MODE="0666", GROUP="dialout"' % port
        )
    ...
    if devices:
        for vendor, product in devices:
            rule_line = (
                ...
                'MODE="0666", GROUP="dialout"' % (vendor, product)
            )
...
def check_setup():
    ...
    for port in TARGET_PORTS:
        port_path = "/dev/%s" % port
        port_check = run_cmd(
            "ls -l %s 2>/dev/null || echo 'Порт %s не найден'" % (port_path, port)
        )
...
if __name__ == "__main__":
    ...
    ports_str = ", ".join("/dev/%s" % p for p in TARGET_PORTS)
    print("Настройка доступа к %s для пользователя '%s'" % (ports_str, TARGET_USER))
    setup_user()
    ...
    check_setup()
    print("\nГотово! '%s' может работать с %s без sudo после перелогина." %
          (TARGET_USER, ports_str))